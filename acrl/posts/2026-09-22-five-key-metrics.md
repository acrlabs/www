---
title: "Five key autoscaling metrics for Kubernetes"
authors:
  - drmorr
datetime: 2026-09-22 11:00:00
template: post.html
---

<figure markdown>
  ![a purple octopus holding a magnifying glass in one tentacle up to his eye; he's holding a sign that reads "mr.
squidler's school 4 kids who can't autoscale gud"](/img/posts/five-key-metrics.png)
  <figcaption>"What is this, a cluster for ants?  It's gotta be at least... 3 times that big!</figcaption>
</figure>

Earlier this year, I had the opportunity to chat with Courtney Nash[^1] at SREcon, and one of the things she reminded me
is that "it's OK to say that you're an expert".  So, what am I an expert in?  Well, for the last decade-plus, I've been
working with autoscalers for distributed systems.  I built [Clusterman](https://engineeringblog.yelp.com/2019/11/open-source-clusterman.html)
at Yelp---originally for Apache Mesos, and then migrated to Kubernetes; I've contributed to both the
[Kubernetes Cluster Autoscaler](https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/FAQ.md) and
[Karpenter](https://karpenter.sh); and I've helped a lot of companies do autoscaling better.  So, I think it's safe to
say that I'm something of an autoscaling expert.
...
or outage, instead of normal operations[^2]?
...
hands on[^3], but then you're gonna get another visit from your VP of Finance.  On the flip side, you can make your
...
"capacity planning".  Unlike today, when you can ask AWS for a new machine and get it approximately instantly[^4], it
...
3. Vertical autoscaling: also called "right-sizing", vertical autoscaling is a less-frequently-used form of
   autoscaling[^5].  If application scaling is answering the question, "How many copies of my application do I need?",
...
"autoscaling" into these three different buckets is to make it easier to reason about[^6].

In an ideal world, however, we'd have a single autoscaler that behaves as follows[^7]:
...
3. Mean Time to First Byte (MTFB) - this is a concept I borrowed from the front-end community, where they have the
   ...
   autoscaler decides to scale up, to that application or service actually doing useful work?[^9].  If capacity and
...
5. Application churn - this final metric is not often talked about, but anybody who's written an autoscaler (or, heck,
   ...
   And then three.  And then two.  And then.... Anyways, go look up [PID controllers](https://en.wikipedia.org/wiki/PID_controller)[^10].

So there you have it!  Track those metrics, optimize your autoscaler for all five of them at the same time, and boom!
You're all done!

Ahahahahah.  Haha!  HAHA!  I can't even say that with a straight face[^11].  Anyways, we're going to take a closer look
at each of these metrics over the next several weeks, and we'll give some examples from our past experience about why
they're important and what can happen if you don't pay attention to them.

As always, thanks for reading.

~drmorr

[^1]: Who is awesome, by the way!

[^2]: I can't count the number of times I've seen or been involved in an incident where the system loses all its
...
[^3]: This is Anthropic's strategy, but for some reason it doesn't seem to be working out for them.

[^4]: Unless it's a GPU-backed node, sigh.

[^5]: However, with the advent of [in-place pod resizing](https://kubernetes.io/docs/tasks/configure-pod-container/resize-container-resources/)
    in Kubernetes 1.35, I expect to see a lot more places experimenting with vertical scaling.

[^6]: If you really want to throw a monkey wrench into things, I also believe that _scheduling_ is the same thing as
    well!  If this weren't true, why does every autoscaler in existence also run a scheduling simulation as a part of
    its feedback loop?

[^7]: Astute readers might notice that this is just "getting rid of some of the layers" which is a concept I haven't
...
[^8]: "But David," I hear you say, "I'm just a lowly platform engineer, and that stuff is controlled by our service mesh
    team!  I can't control how we do retries!"  To which I respond, "Don't make me tap the 'layers' sign."

[^9]: Most of the other articles I've read about this look at "mean time to scheduling", e.g., how long between an
...
[^10]: I've been having a lot of conversations recently with folks about how the software engineering community refuses
...
[^11]: Also I really hope you read that in your best [Claptrap](https://en.wikipedia.org/wiki/Claptrap) impersonation,
    because I sure did.
